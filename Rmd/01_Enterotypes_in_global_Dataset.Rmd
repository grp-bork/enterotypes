...
p3.studies <- df %>% 
 ...
 mutate(fkm_k3 = factor(fkm_k3, levels = c("Firmicutes", "Bact_Phoc", "Prevotella"))) %>%  # arrange for order by Firmicutes
  ...
  geom_col()+
  geom_text(aes(label = SampleNumber),position = position_stack(vjust = 1), hjust = -0.1, size = 6)+
...
study.p <- ggarrange(p1,p3.studies,  ncol = 2, widths = c(2,1), align = "h", common.legend = TRUE)
study.p

```

split the plot onto two:
```{r eval=FALSE, include=FALSE}
library(tidyverse)
library(patchwork)
...
# ---- Define study order based on Firmicutes proportion ----
study_order <- df %>% 
  filter(fkm_k3 == "Firmicutes") %>% 
  arrange(n_prob) %>% 
  pull(studies)

# In case some studies do not have Firmicutes, add them at the end
study_order <- c(
  study_order,
  setdiff(unique(df$studies), study_order)
)

# Split into two A4 pages
study_split <- split(
  study_order,
  ceiling(seq_along(study_order) / ceiling(length(study_order) / 2))
)
make_plot <- function(study_subset) {
  
  df_sub <- df %>% 
    filter(studies %in% study_subset) %>% 
    mutate(
      studies = factor(studies, levels = rev(study_subset)),
      fkm_k3 = factor(fkm_k3, levels = c("Prevotella", "Firmicutes", "Bact_Phoc")),
      ET_count = as.factor(et_count)
    )
...
plot_page1 <- make_plot(study_split[[1]])
plot_page2 <- make_plot(study_split[[2]])
plot_page1
plot_page2

ggsave(
  here("figures/enterotype_study_plot_page1.pdf"),
  plot_page1,
  height = 11.69,
  width = 8.27,
  units = "in"
...
ggsave(
  here("figures/enterotype_study_plot_page2.pdf"),
  plot_page2,
  height = 11.69,
  width = 8.27,
  units = "in"
